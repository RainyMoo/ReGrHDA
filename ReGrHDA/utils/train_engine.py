        ...
        for step, (
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                if __C.ITC_LOSS == 'True':
                    pred, region, grid, region_pred, grid_pred, itc_loss1, itc_loss2= net(
                        sub_frcn_feat_iter,
                        sub_grid_feat_iter,
                        sub_bbox_feat_iter,
                        sub_grid_bbox_feat_iter,
                    ...
                    loss_region = loss_fn(region_pred, loss_item[1])
                    loss_grid = loss_fn(grid_pred, loss_item[1])
                    
                    loss_clip1 = info_nce_loss(pred3, pred1)
                    loss_clip2 = info_nce_loss(pred3, pred2)
                    
                    kl_loss_01 = loss_kl(F.log_softmax(region,dim=1), F.softmax(Variable(grid),dim=1))
                    kl_loss_10 = loss_kl(F.log_softmax(grid,dim=1), F.softmax(Variable(region),dim=1))
                    alpha = 1
                    loss = (loss_main + loss_region +loss_grid)/3 + (kl_loss_01 + kl_loss_10)/2 + (itc_loss1 + itc_loss2)/2
